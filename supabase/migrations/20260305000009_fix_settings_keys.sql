-- Rename store_name to restaurant_name to match app conventions
UPDATE settings SET key = 'restaurant_name' WHERE key = 'store_name';

-- Update the initial_setup function to use correct setting keys
...
BEGIN
    ...
    IF v_admin_count > 0 THEN
        RETURN jsonb_build_object('success', false, 'message', 'Setup already completed. An admin user already exists.');
    END IF;
    ...
    -- Insert default settings using keys that match the frontend app
    INSERT INTO settings (org_id, key, value, description) VALUES
        (v_org_id, 'restaurant_name', p_store_name, 'Business/restaurant name'),
        ...
        (v_org_id, 'service_charge', '0', 'Service charge percentage'),
        (v_org_id, 'theme', 'light', 'UI theme'),
        (v_org_id, 'touch_mode', 'false', 'Touch screen mode'),
        (v_org_id, 'language', 'en', 'Interface language'),
        (v_org_id, 'receipt_header', 'Thank you for dining with us!', 'Receipt header text'),
        (v_org_id, 'receipt_footer', 'Visit again soon!', 'Receipt footer text'),
        (v_org_id, 'industry_mode', 'restaurant', 'Industry mode');
