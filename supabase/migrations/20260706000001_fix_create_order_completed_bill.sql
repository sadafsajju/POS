-- create_order's KOT bill-finder attached new KOTs to bills with status
-- 'completed' (its filter only excluded 'paid'/'cancelled', while clear_table
-- marks cleared bills 'completed' and the POS active-bill lookup excludes
-- 'completed'). On any table that had been cleared, a new dine-in KDS order
-- became a KOT under the old completed bill — invisible to the POS, so the
-- payment overlay could never open. Redefine create_order (same signature as
-- migration 25) with the finder excluding 'completed' so a cleared bill can
-- never accept new KOTs.
...
BEGIN
    ...
    ELSIF p_create_as_kot AND p_table_id IS NOT NULL AND p_order_type = 'dine_in' THEN
        SELECT id INTO v_bill_id FROM orders
            WHERE table_id = p_table_id AND status NOT IN ('paid', 'completed', 'cancelled')
