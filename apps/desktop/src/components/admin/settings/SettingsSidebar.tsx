...
const settingsSections = [
  ...
  },
  {
    ...
    description: 'Tax, allergens, tipping, GDPR',
    href: '/admin/more/financial',
...
export function SettingsSidebar() {
  ...
  const isActiveRoute = (href: string) => {
    if (href === '/admin/more/menu') {
      return location.pathname.startsWith('/admin/more/menu')
    }
    return location.pathname === href
  }

  const visibleReports = reportsSections.filter(s => {
    if (s.requires === 'tipping') return settings.tippingEnabled
    if (s.requires === 'uk_vat') return settings.taxRegime === 'uk_vat'
    return true
  })
  ...
  return (
    <div className="w-60 bg-zinc-900 border-r border-zinc-800 flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-zinc-800">
        <span className="text-lg font-bold tracking-tight text-zinc-300">More</span>
      </div>

      {/* Navigation */}
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto scrollbar-hide">
        <p className="text-[10px] uppercase tracking-wider text-zinc-600 px-3 mb-2 font-bold">Insights</p>
        ...
        <div className="pt-4 pb-2 px-3">
          ...
          <p className="text-[10px] uppercase tracking-wider text-zinc-600 mt-3 font-bold">Settings</p>
        </div>
